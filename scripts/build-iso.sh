...
if [ -f "$BIOS_GRUB" ]; then
    ...
    sed -i 's|linux /images/pxeboot/vmlinuz |linux /images/pxeboot/vmlinuz inst.ks=cdrom:/ks.cfg |g' "$BIOS_GRUB"
fi
...
if [ -f "$EFI_GRUB" ]; then
    ...
    sed -i 's|linuxefi /images/pxeboot/vmlinuz |linuxefi /images/pxeboot/vmlinuz inst.ks=cdrom:/ks.cfg |g' "$EFI_GRUB"
fi
